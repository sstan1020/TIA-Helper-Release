...
| `tia_pin` | 连接到项目，并且**保持连着**（不用完就断），适合频繁切换项目时用 |
| `tia_unpin` | 取消"保持连接"，并断开这个项目 |
| `tia_listpinned` | 看看现在有哪些项目在"保持连接"状态 |
| `tia_status` | 看看现在连的是哪个项目/PLC |
| `tia_detach` | 断开连接 |
| `tia_hwcode` | 看看这台电脑的授权码（硬件码） |
| `tia_exportlist` | 列出所有可以导出的程序块 |
| `tia_listplcs` | 列出这个项目里所有的 PLC（一个项目可能有好几个） |
| `tia_selectplc` | 切换要操作哪个 PLC（项目里有多个 PLC 时用） |
| `tia_exportdestination` | 看看导出/导入现在默认存到哪个文件夹 |
...
正常情况下每次调用工具，AI 用完就自动断开连接，下次用又要重新连一次。如果你会
**频繁在两个项目之间切换**，可以让 AI 用 `tia_pin` 把某个项目"钉住"，保持连接：

> **你：** 帮我把 X_ZIP 和 Project1 这两个项目都连上，之后我会经常在两个之间切换。
>
> **AI：** *(依次调用 `tia_pin` keyword="X_ZIP"，再调用 `tia_pin` keyword="Project1")*
> 两个项目都已经连上并保持连接了：
> - X_ZIP.ap20 → PLC_1
> - Project1.ap20 → xPLC_1x
>
> 之后你让我操作哪个项目，我都能瞬间切换过去，不用重新连接。

之后正常用 `tia_attach` 切换到某个项目即可，因为已经 pin 过了，AI 会发现这个项目
"秒连"，不需要重新走一遍连接流程。不需要一直保持连接的话，让 AI 调用
`tia_unpin` 取消就行。

### 场景二：一个项目里有好几个 PLC

有些项目不是只有一个 PLC，而是好几个 PLC 设备放在同一个项目里：

> **你：** 这个项目里有几个 PLC？我想导出第二个 PLC 的程序块列表。
>
> **AI：** *(调用 `tia_listplcs`)* 这个项目有两个 PLC：`PLC_1`（当前选中）和
> `PLC_2`。*(调用 `tia_selectplc` plc="PLC_2"，再调用 `tia_exportlist`)*
> 已经切换到 PLC_2，这是它的程序块列表：...

如果项目只有一个 PLC（绝大多数情况），AI 不会主动问你选哪个，直接操作那一个就行。
